...
function register(src, entryPath, dtsPath, group) {
    ...
    for (const m of src.matchAll(/export\s+\{([^}]*)\}/g)) {
        for (const n of m[1].split(',')) names.add(n.trim().split(/\s+as\s+/).pop());
    }
    ...
    const def = /export\s+default\s+(?:function|class)\s+(\w+)/.exec(src)?.[1] ?? /export\s+default\s+(\w+);/.exec(src)?.[1];
    if (def) {
        names.add(def);
    ...
    for (const n of names) {
        if (pascal.test(n)) writeFileSync(`dist/component-docs/${n}.md`, `---\ncategory: ${group}\n---\n`);
    }
}
const titleCase = (base) => base.replace(/(^|-)(\w)/g, (_, __, c) => c.toUpperCase());
...
for (const f of files) {
    ...
    register(src, existsSync(shim) ? `../.design-sync/shims/${base}` : `../${comps}/${f}`, `./components/${f}`, f.startsWith('ui/') ? titleCase(base) : 'App');
}
...
const dsFiles = readdirSync(dsDir).filter((f) => f.endsWith('.tsx')).sort();
for (const f of dsFiles) {
    const base = f.replace(/\.tsx$/, '');
    register(readFileSync(join(dsDir, f), 'utf8'), `../${dsDir}/${base}`, `./ds/${base}`, titleCase(base));
}
writeFileSync('dist/entry.ts', entryLines.join('\n') + '\n');
try {
    execSync('pnpm exec tsc -p .design-sync/tsconfig.dts.json', { stdio: 'inherit' });
} catch { /* declaration emit continues past type errors */ }
try {
    execSync('pnpm exec tsc -p .design-sync/tsconfig.dts-ds.json', { stdio: 'ignore' });
} catch { /* TS6059 (rootDir vs button.tsx importado) é esperado; os .d.ts saem mesmo assim */ }
writeFileSync('dist/types/index.d.ts', dtsLines.join('\n') + '\n');
execSync('.ds-sync/node_modules/.bin/tailwindcss -i .design-sync/tailwind-input.css -o dist/styles.css', { stdio: 'inherit' });
...
for (const p of ['outfit', 'raleway']) {
    ...
    for (const f of readdirSync(dir)) if (f.endsWith('.woff2')) cpSync(join(dir, f), join('dist/files', f));
}
// O conversor espera um "pacote" com nome; dist/ faz esse papel (o package.json da raiz não tem name).
writeFileSync('dist/package.json', JSON.stringify({ name: 'upay', version: '0.0.0', types: 'types/index.d.ts', module: 'entry.ts' }, null, 2));
