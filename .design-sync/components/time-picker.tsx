...
function TimeColumns({
    ...
    const item = (selected: boolean) =>
        cn(
            'hover:bg-muted focus-visible:ring-ring/50 flex h-8 w-full shrink-0 items-center justify-center rounded-full text-sm tabular-nums transition-colors outline-none focus-visible:ring-[3px]',
            selected &&
    ...
    return (
        <div
            ...
            className={cn('flex h-full max-h-80 min-h-56 gap-2', className)}
        >
            ...
            <div className={column}>
                ...
                <div
                    role="listbox"
                    aria-label={labels?.minutes ?? 'Minutos'}
                    className={listbox}
                >
                    {minutes.map((m) => (
                        <button
        ...
        <Popover open={isOpen} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button
                    ...
                    className={cn(
                        'data-empty:text-muted-foreground w-40 justify-start font-normal tabular-nums',
                        className,
                    )}
                >
