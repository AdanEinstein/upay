...
- [x] 2.1 FEITO 2026-09-21 (comparação estática código × protótipo, sem app rodando; Início, Vendas, NovaVenda, Clientes, Produtos, Catálogo, Financeiro, Config). Correções: `sales/show.tsx` parcela que vence hoje → "Vence hoje"; `payables.tsx` rótulo "Venceu/Vence hoje/Vence {data}" (chaves `payables.due.*`). Mantido de propósito: sem skeleton/erro de carregamento (páginas Inertia síncronas), sem layout desktop do lojista (app é mobile-only), sem "Cobrar todos" (sem backend), extras do app (busca em Vendas, selo "Publicado", chips de atalho no Início no lugar da engrenagem). Comparação de cores/espaçamento fina só com o app rodando.
- [~] 2.3 Desktop do lojista (≥1024px, `lg:`) implementado 2026-09-21 e NÃO validado no navegador: `components/shop/sidebar.tsx` (224px) + coluna de 800px em `shop-layout.tsx`, BottomNav some, `BottomSheet` vira modal 440px, e ajustes `lg:` em todas as páginas do lojista (Nova venda como modal/card, forms como card centralizado; sem modal real nem rotas novas). Não feito: skeleton/erro, "Cobrar todos", prévia do catálogo em modal.
...
- [~] 4.6 Reimportada 2026-09-21: mobile 3.1–3.8 já conferia; o protótipo ganhou seção **desktop** das páginas públicas, implementada com `lg:` e NÃO validada no navegador: `public/debt.tsx` (lista em coluna de 520px sobre `bg-muted`; detalhe da compra e PIX viram modal duplo 440+340px, clique no fundo fecha, sem Escape), `public/catalog.tsx` (capa de 120px em largura total, conteúdo 900px, grades de 5 colunas, produto em 2 colunas 400px + info com botão WhatsApp inline). `invalid`/`catalog-unavailable` sem mudança (só o ícone é 8px maior no desktop).
...
- [x] 6.5 Atualizar este arquivo e a memória `project_design_import_phases.md` ao concluir cada etapa.
