...
export default function CustomerForm({ customer }: { customer: Customer | null }) {
    ...
    const [more, setMore] = useState(Boolean(customer?.document || customer?.address));
    const form = useForm({
        name: customer?.name ?? '',
    ...
    const title = customer ? t('customers.form.editTitle') : t('customers.form.newTitle');
    ...
    return (
        <>
            ...
            <form onSubmit={submit} className="flex flex-col gap-3.5 px-5 pt-2 lg:bg-card lg:border-border lg:mx-auto lg:mt-6 lg:w-[460px] lg:rounded-2xl lg:border lg:p-7">
                ...
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="phone">{t('customers.form.whatsapp')}</Label>
                    <Input id="phone" type="tel" inputMode="tel" value={form.data.phone} onChange={(event) => form.setData('phone', event.target.value)} placeholder={t('customers.form.whatsappPlaceholder')} className="h-11" />
                    <InputError message={form.errors.phone} />
                </div>
                <div className="flex flex-col gap-1.5">
                    <Label htmlFor="notes">{t('customers.form.notes')}</Label>
                    <Textarea id="notes" value={form.data.notes} onChange={(event) => form.setData('notes', event.target.value)} placeholder={t('customers.form.notesPlaceholder')} />
                    <InputError message={form.errors.notes} />
                </div>
                <button type="button" onClick={() => setMore(!more)} className="text-brand flex items-center gap-1.5 py-1 text-[13.5px] font-semibold">
                    {t('customers.form.more')}
                    <CaretDownIcon className={cn('size-3.5 transition-transform', more && 'rotate-180')} weight="bold" />
                </button>
                {more && (
                    <>
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="document">{t('customers.form.document')}</Label>
                            <Input id="document" inputMode="numeric" value={form.data.document} onChange={(event) => form.setData('document', event.target.value)} placeholder={t('customers.form.documentPlaceholder')} className="h-11" />
                            <InputError message={form.errors.document} />
                        </div>
                        <div className="flex flex-col gap-1.5">
                            <Label htmlFor="address">{t('customers.form.address')}</Label>
                            <Input id="address" value={form.data.address} onChange={(event) => form.setData('address', event.target.value)} placeholder={t('customers.form.addressPlaceholder')} className="h-11" />
