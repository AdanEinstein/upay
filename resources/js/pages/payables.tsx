...
type Row = { id: number; description: string; dueDate: string; amountCents: number };
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-4 px-5 pt-2">
                {empty && <p className="text-muted-foreground py-10 text-center text-sm">{t('payables.empty')}</p>}
                {ORDER.filter((key) => groups[key].length > 0).map((key) => (
                    <section key={key} className="flex flex-col gap-2">
                        <h2 className={cn('text-[12.5px] font-bold tracking-wide uppercase', key === 'overdue' ? 'text-destructive' : 'text-muted-foreground')}>
                            {t(`payables.groups.${key}`)}
                        </h2>
                        {groups[key].map((row) => (
                            <div key={row.id} className="border-border flex items-center gap-2.5 rounded-xl border px-3 py-2.5">
                                <button
                                    type="button"
                                    onClick={() => router.post(pay.url({ expense: row.id }), {}, { preserveScroll: true })}
