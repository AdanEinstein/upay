...
export default function PixKey({
    ...
    pixKeyType: (typeof TYPES)[number];
    pixKey: string;
}) {
    const { t } = useTranslation('shop');
    const form = useForm({
        pix_key_type: pixKeyType,
        pix_key: maskPixKey(pixKeyType, pixKey),
    });
    ...
    function changeType(type: PixKeyType) {
        form.setData((data) => ({
            ...data,
            pix_key_type: type,
            pix_key: maskPixKey(type, data.pix_key),
        }));
    }
    ...
    return (
        <>
            ...
            <form
                onSubmit={submit}
                className="flex flex-col gap-4 px-5 pt-2 lg:max-w-[440px]"
            >
                <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {TYPES.map((key) => (
                            <Chip
                                key={key}
                                active={form.data.pix_key_type === key}
