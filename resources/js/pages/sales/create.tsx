...
import { CameraIcon, CaretLeftIcon, CheckIcon, MinusIcon, PlusIcon } from '@phosphor-icons/react';
...
function splitInstallments(totalCents: number, count: number, firstDue: string) {
...
export default function CreateSale({
    ...
    const totalCents = lines.reduce((sum, item) => sum + item.priceCents * cart[item.key], 0);
    const itemCount = lines.reduce((sum, item) => sum + cart[item.key], 0);
    const itemCountLabel = t('newSale.products.itemCount', { count: itemCount });
    const entry = Math.min(downPayment ?? 0, totalCents);
    const plan = splitInstallments(totalCents - entry, installments, firstDue);

    const needle = customerQuery.trim().toLowerCase();
    const visibleCustomers = customers.filter((item) => item.name.toLowerCase().includes(needle));
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-2.5 px-4 py-3 lg:px-5 lg:pt-0">
                ...
                <div className="flex flex-1 gap-1.5">
                    {[1, 2, 3].map((segment) => (
                        <span
                            key={segment}
                            className={cn('h-1 flex-1 rounded-sm', step >= segment ? 'bg-brand' : 'bg-border')}
                        />
                    ))}
                </div>
            </div>
            ...
            {step === 1 && (
                <>
                    <div className="flex flex-1 flex-col gap-4 px-5 pb-4">
                        <h1 className="font-heading text-[19px] font-bold">{t('newSale.customer.title')}</h1>
                        <Input
                            value={customerQuery}
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-2">
                                {visibleCustomers.slice(0, 20).map((item) => (
                                    <button
                                        key={item.id}
                                        type="button"
                                        ...
                                        className={cn(
                                            ...
                                            item.id === customerId ? 'border-brand bg-brand-soft' : 'border-border bg-card',
                                        )}
                                    >
                                        ...
                                        {item.id === customerId && <CheckIcon className="text-brand size-[18px]" weight="bold" />}
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        {showNewCustomer && (
                            <form
                                onSubmit={(event) => {
                                    ...
                                    newCustomer.post(storeCustomer.url());
                                }}
                                className="bg-card border-border flex flex-col gap-2.5 rounded-2xl border p-3.5"
                            >
                                <Input
                                    value={newCustomer.data.name}
                                    onChange={(event) => newCustomer.setData('name', event.target.value)}
                                    placeholder={t('newSale.customer.name')}
                                    aria-label={t('newSale.customer.name')}
                                    className="h-10"
                                />
                                <InputError message={newCustomer.errors.name} />
                                <Input
                                    value={newCustomer.data.phone}
                                    onChange={(event) => newCustomer.setData('phone', event.target.value)}
                                    placeholder={t('newSale.customer.whatsapp')}
                                    aria-label={t('newSale.customer.whatsapp')}
                                    inputMode="tel"
                                    className="h-10"
                                />
                                <InputError message={newCustomer.errors.phone} />
                                <Button type="submit" disabled={newCustomer.processing} className="h-10">
            ...
            {step === 2 && (
                <>
                    <div className="flex flex-1 flex-col gap-3.5 px-5 pb-4">
                        <h1 className="font-heading text-[19px] font-bold">{t('newSale.products.title')}</h1>
                        <Input
                            value={productQuery}
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="px-2.5 pt-2 pb-2.5">
                                            <p className="mb-0.5 text-[12.5px] leading-tight font-semibold">{item.name}</p>
                                            <p className="text-muted-foreground text-[12.5px]">
                                                {item.stock === 0 ? t('newSale.products.outOfStock') : money(item.priceCents)}
                                            </p>
                                        </div>
                                        {item.stock > 0 &&
                                            (quantity > 0 ? (
                    ...
                    <div className="bg-background border-border sticky bottom-0 mt-auto flex items-center gap-3 border-t px-5 py-3 lg:static lg:bg-transparent">
                        ...
                        <Button size="lg" className="h-12 px-6 text-base" disabled={itemCount === 0} onClick={() => setStep(3)}>
                            {t('newSale.continue')}
                        </Button>
                    </div>
                </>
            ...
            {step === 3 && (
                <>
                    <div className="flex flex-1 flex-col gap-4 px-5 pb-4">
                        <h1 className="font-heading text-[19px] font-bold">{t('newSale.payment.title')}</h1>
                        <div className="flex gap-2">
                            {(['avista', 'fiado', 'parcelado'] as const)
                                .filter((type) => type === 'avista' || customerId !== null)
                                .map((type) => (
                                    <Chip key={type} active={paymentType === type} onClick={() => setPaymentType(type)} className="px-4 py-2 text-[13.5px]">
                                        {t(`newSale.payment.types.${type}`)}
                                    </Chip>
                                ))}
                        </div>
                            ...
                            <div className="bg-card border-border flex flex-col gap-3 rounded-2xl border p-3.5 lg:bg-background">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="min-w-4 text-center text-[15px] font-bold">{installments}</span>
                                        <Button
                                            type="button"
                                            variant="outline"
                                            size="icon-sm"
                                ...
                                <div className="flex flex-col gap-1.5">
                                    <Label htmlFor="down-payment">{t('newSale.payment.downPayment')}</Label>
                                    <MoneyInput id="down-payment" cents={downPayment} onCentsChange={setDownPayment} placeholder="R$ 0,00" className="h-10" />
                                </div>
                                <div className="flex flex-col gap-1.5">
                                    <Label htmlFor="first-due">{t('newSale.payment.firstDue')}</Label>
                                    <Input id="first-due" type="date" min={isoToday()} value={firstDue} onChange={(event) => setFirstDue(event.target.value)} className="h-10" />
                                    <InputError message={saleErrors.first_due_date} />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <p className="text-muted-foreground mb-0.5 text-[12.5px] font-semibold">{t('newSale.payment.preview')}</p>
                                    {plan.map((item) => (
                                        <div key={item.number} className="border-border flex justify-between border-t py-1 text-[13px]">
                                            <span>{t('newSale.payment.previewLine', { number: item.number, date: shortDate(item.date) })}</span>
                                            <span className="font-semibold">{money(item.cents)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div>
                            <p className="text-muted-foreground mb-2 text-[13px] font-semibold">{t('newSale.payment.method')}</p>
                            <div className="flex gap-2">
                                {METHODS.map((item) => (
                                    <Chip key={item} active={method === item} onClick={() => setMethod(item)} className="px-4 py-2 text-[13.5px]">
                                        {t(`methods.${item}`)}
                                    </Chip>
                                ))}
                            </div>
                        ...
                        <div className="bg-muted flex flex-col gap-1.5 rounded-2xl p-3.5">
                            <div className="text-muted-foreground flex justify-between text-[13px]">
                                <span>{t('newSale.payment.summaryCustomer')}</span>
                                <span className="text-foreground font-medium">{customer?.name ?? t('newSale.payment.noCustomer')}</span>
                            </div>
                            <div className="text-muted-foreground flex justify-between text-[13px]">
                                <span>{t('newSale.payment.summaryItems')}</span>
                                <span className="text-foreground font-medium">{itemCountLabel}</span>
                            </div>
                            <div className="border-border flex justify-between border-t pt-1.5 text-[15px] font-bold">
                                <span>{t('newSale.payment.summaryTotal')}</span>
