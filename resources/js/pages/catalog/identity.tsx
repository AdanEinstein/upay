...
function ImagePicker({ label, preview, onPick, className }: { label: string; preview: string | null; onPick: (file: File) => void; className?: string }) {
    return (
        <div>
            <p className="text-muted-foreground mb-1.5 text-[12.5px] font-semibold">{label}</p>
            <label className={cn('border-border text-muted-foreground flex h-20 w-full cursor-pointer items-center justify-center overflow-hidden rounded-xl border-2 border-dashed hover:border-primary/50 has-[:focus-visible]:ring-ring/50 transition-colors has-[:focus-visible]:ring-[3px]', className)}>
                <input
                    type="file"
                    accept="image/*"
                    className="sr-only"
...
export default function CatalogIdentity({ name, link, logoUrl, coverUrl, welcomeText, whatsapp, accentColor, palette }: Props) {
    ...
    return (
        <>
            ...
            <form onSubmit={submit} className="flex flex-col gap-4 px-5 pt-2 lg:grid lg:grid-cols-2 lg:items-start lg:gap-8">
                ...
                <div className="border-border overflow-hidden rounded-2xl border">
                    ...
                    <div className="bg-card flex items-center gap-2 px-3 py-2.5">
                        {logoPreview ? (
                            <img src={logoPreview} alt="" className="size-7 rounded-lg object-cover" />
                        ) : (
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    <ImagePicker label={t('catalog.identityForm.logo')} preview={logoPreview} onPick={(file) => form.setData('logo', file)} />
                    <ImagePicker label={t('catalog.identityForm.cover')} preview={coverPreview} onPick={(file) => form.setData('cover', file)} />
                </div>
                ...
                <Button type="submit" size="lg" className="order-2 h-12 text-base lg:self-start lg:px-7" disabled={form.processing}>
