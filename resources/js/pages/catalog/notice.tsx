...
export default function CatalogNotice({
    ...
    function submit(event: FormEvent) {
        ...
        form.transform((data) => ({
            ...data,
            notice_expires_on: data.notice_expires_on || null,
        }));
        form.put(update.url());
    }
    ...
    return (
        <>
            ...
            <PageHeader
                title={t('catalog.noticeForm.title')}
                back={show.url()}
            />
            ...
            <form onSubmit={submit} className="flex flex-col gap-4 px-5 pt-2">
                <div>
                    <p className="text-muted-foreground mb-2 text-[13px] font-semibold">
                        {t('catalog.noticeForm.previewLabel')}
                    </p>
                    <div
                        className={cn(
                            'min-h-10 rounded-[10px] px-3 py-2.5 text-[13px] font-semibold',
                            NOTICE_STYLES[form.data.notice_type],
                        )}
                    >
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex justify-between">
                        <Label htmlFor="notice-text">
                            {t('catalog.noticeForm.text')}
                        </Label>
                        <span className="text-muted-foreground text-xs">
                            {form.data.notice_text.length}/120
                        </span>
                    </div>
                    <Textarea
                        id="notice-text"
                        maxLength={120}
                        value={form.data.notice_text}
                        onChange={(event) =>
                            form.setData('notice_text', event.target.value)
                        }
                        placeholder={t('catalog.noticeForm.placeholder')}
                        className="min-h-16"
                    />
                ...
                <div>
                    <p className="text-muted-foreground mb-2 text-[13px] font-semibold">
                        {t('catalog.noticeForm.type')}
                    </p>
                    <div className="flex gap-2">
                        {(Object.keys(NOTICE_STYLES) as NoticeType[]).map(
                            (key) => (
                                <Chip
                                    key={key}
                                    active={form.data.notice_type === key}
                                    onClick={() =>
                                        form.setData('notice_type', key)
                                    }
                                >
                ...
                <label className="flex items-center gap-2.5 text-sm">
                    <Checkbox
                        checked={form.data.notice_active}
                        onCheckedChange={(checked) =>
                            form.setData('notice_active', checked === true)
                        }
                    />
