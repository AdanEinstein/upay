...
export default function CatalogPreview({
    ...
    return (
        <>
            ...
            <PageHeader
                title={t('catalog.previewPage.title')}
                back={show.url()}
            />
            ...
            <div className="flex flex-col gap-4 px-5">
                <div
                    className="mt-3 h-[90px] overflow-hidden rounded-2xl"
                ...
                <div className="-mt-[30px] flex items-center gap-2.5 pl-3">
                    <div className="bg-card border-background text-muted-foreground flex size-14 items-center justify-center overflow-hidden rounded-xl border-[3px] text-[11px]">
                        {store.logoUrl ? (
                            ...
                            t('catalog.previewPage.logo')
                        )}
                    </div>
                    <span className="mt-6 text-[15px] font-bold">
                        {store.name}
                    </span>
                </div>
                {store.welcomeText && (
                ...
                {promotions.length > 0 && (
                    <>
                        <p className="text-muted-foreground text-[12.5px] font-bold uppercase">
                            {t('catalog.previewPage.promotions')}
                        </p>
                        <div className="flex gap-2.5 overflow-x-auto">
                            {promotions.map((promotion) => (
                                <div
                                    key={promotion.id}
                                    className="border-border w-[120px] shrink-0 overflow-hidden rounded-xl border lg:w-[180px]"
                                >
                                    <ProductImage
                ...
                <p className="text-muted-foreground text-[12.5px] font-bold uppercase">
                    {t('catalog.previewPage.products')}
                </p>
            ...
            <div className="bg-background border-border sticky bottom-0 mt-6 border-t px-5 py-3 lg:static lg:border-t-0 lg:px-5">
                <Button
                    size="lg"
                    className="h-12 w-full text-base lg:w-auto lg:px-8"
                    onClick={() => router.put(publish.url())}
                >
