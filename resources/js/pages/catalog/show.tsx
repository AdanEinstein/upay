...
type Props = { url: string; published: boolean; activePromotions: number; noticeLive: boolean };

export default function CatalogShow({ url, published, activePromotions, noticeLive }: Props) {
        ...
        <>
            ...
            <div className="flex flex-col gap-[18px] px-5 pt-2">
                <div className="bg-muted flex flex-col gap-2.5 rounded-2xl p-3.5 lg:grid lg:grid-cols-[1fr_auto] lg:gap-x-4 lg:gap-y-0.5 lg:p-[18px]">
                    ...
                    <p className="text-sm font-semibold break-all">{url.replace(/^https?:\/\//, '')}</p>
                ...
                <div className="flex flex-col gap-2 lg:grid lg:grid-cols-2 lg:gap-3.5">
                    {rows.map(({ icon: Icon, title, hint, href }) => (
                        <Link key={href} href={href} className="border-border bg-card flex items-center gap-3 rounded-2xl border p-3.5 lg:gap-3.5 lg:p-[18px]">
                            <span className="bg-brand-soft text-brand flex size-9 shrink-0 lg:size-10 items-center justify-center rounded-[10px]">
