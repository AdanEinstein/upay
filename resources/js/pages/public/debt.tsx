...
export default function Debt({
    ...
    const installment =
        purchase?.installments.find((item) => item.id === pixId) ?? null;
    ...
    if (openCents === 0) {
        return (
            <>
                {title}
                <MobileScreen>
                    <p className="text-muted-foreground px-5 pt-4 text-[13px]">
                        {store}
                    </p>
                    <EmptyState
                        icon={<CheckCircleIcon />}
                        title={t('allPaidTitle')}
                        description={t('allPaidDescription', {
                            name: customer,
                            store,
                        })}
                    />
        ...
        <>
            ...
            <div className={cn('lg:bg-muted', purchase && 'hidden lg:block')}>
                <MobileScreen className="gap-4 px-5 pt-4 pb-8 lg:max-w-[520px] lg:gap-5 lg:bg-transparent lg:pt-14">
                    ...
                    <div className="bg-muted lg:bg-card rounded-2xl p-4 lg:p-5">
                        ...
                        <p className="text-[26px] font-bold tabular-nums lg:text-3xl">
                            {money(openCents)}
                        </p>
                        {(overdue.length > 0 || nextDue) && (
                            <p
                                className={
                                    overdue.length > 0
                                        ? 'text-destructive mt-1.5 text-[12.5px]'
                                        : 'text-muted-foreground mt-1.5 text-[12.5px]'
                                }
                            >
                                {[
                                    overdue.length > 0
                            ...
                            {purchases.map((item) => (
                                <li key={item.id}>
                                    <button
                                        ...
                                        className={cn(
                                            'lg:bg-card flex w-full items-center gap-2.5 rounded-2xl border p-3 text-left lg:gap-3 lg:border-transparent lg:px-4 lg:py-3.5',
                                            item.status === 'paid' &&
                                                'opacity-70',
                                        )}
                                    >
            ...
            {purchase && (
                <div
                    role="dialog"
                    aria-modal="true"
                    aria-label={t('purchaseTitle', {
                        date: date(purchase.soldAt),
                    })}
                    onClick={(event) =>
                ...
                >
                    <div
                        className={cn(
                            paneClass,
                            'gap-4',
                            installment && 'hidden lg:flex',
                        )}
                    >
                        <Header
                            label={t('purchaseTitle', {
                                date: date(purchase.soldAt),
                            })}
                        ...
                        <dl className="grid grid-cols-3 gap-2 text-center">
                            <Figure
                                label={t('total')}
                                value={money(purchase.totalCents)}
                            />
                            <Figure
                                label={t('alreadyPaid')}
                                value={money(purchase.paidCents)}
                                        ...
                                        <div className="flex items-center gap-2.5">
                                            <div className="flex-1">
                                                <p className="text-[13.5px] font-medium">
                                                    {t('installmentOf', {
                                                        number: item.number,
                                                        total: purchase.installmentCount,
                                                    })}
                                                </p>
...
function Figure({
    ...
    return (
        <div>
            ...
            <dd className={`mt-0.5 text-sm font-bold ${className ?? ''}`}>
                {value}
            </dd>
        </div>
    );
        ...
        <div className={cn(paneClass, 'lg:w-[340px] lg:self-center')}>
            ...
            <div className="flex flex-col items-center gap-3.5 pt-2 text-center">
                ...
                <p className="text-[28px] font-bold tabular-nums">
                    {money(installment.remainingCents)}
                </p>
                {installment.pixQr && (
                    <img
                ...
                <p className="text-muted-foreground max-w-[280px] text-[11.5px]">
                    {t('pixConfirmation')}
                </p>
