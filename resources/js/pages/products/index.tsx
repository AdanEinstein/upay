...
import { ListIcon, PackageIcon, PlusIcon, SquaresFourIcon } from '@phosphor-icons/react';
...
export default function ProductsIndex({ products }: { products: Product[] }) {
    ...
    const categories = [...new Set(products.map((product) => product.category).filter((item): item is string => Boolean(item)))];
    ...
    const badge = (product: Product) =>
        product.stock === 0
            ? { label: t('products.badge.out'), className: 'bg-destructive/15 text-destructive' }
            : product.stock <= product.minStock
              ? { label: t('products.badge.low'), className: 'bg-chart-4/20 text-chart-4' }
              : null;
    ...
    if (products.length === 0) {
        return (
            <>
                ...
                <EmptyState icon={<PackageIcon />} title={t('products.empty.title')} description={t('products.empty.description')}>
                    <Button asChild size="lg" className="h-12 px-6 text-[15px]">
                        <Link href={create.url()}>{t('products.empty.action')}</Link>
                    </Button>
                </EmptyState>
            </>
        );
        ...
        <>
            ...
            <div className="flex flex-col gap-2.5 px-5 lg:flex-row lg:items-center lg:gap-2.5">
                ...
                <ChipRow>
                    ...
                    {categories.map((item) => (
                        <Chip key={item} active={category === item} onClick={() => setCategory(category === item ? null : item)}>
                            {item}
                        </Chip>
                    ))}
            ...
            {visible.length === 0 && (
                <p className="text-muted-foreground py-10 text-center text-sm">{t('common.noResults')}</p>
            )}
            ...
            {view === 'grid' ? (
                <div className="grid grid-cols-2 gap-3 px-5 pt-2 lg:grid-cols-4 lg:gap-4">
                    {visible.map((product) => {
                        ...
                        return (
                            <Link key={product.id} href={show.url({ product: product.id })} className="border-border bg-card relative overflow-hidden rounded-2xl border">
                                {flag && (
                                    <span className={cn('absolute top-2 left-2 z-10 rounded-full px-2 py-0.5 text-[10.5px] font-bold', flag.className)}>
                                        {flag.label}
                                    </span>
                                )}
                ...
                <ul className="flex flex-col gap-2 px-5 pt-2">
                    {visible.map((product) => {
                        ...
                        return (
                            <li key={product.id}>
                                <Link href={show.url({ product: product.id })} className="border-border flex items-center gap-2.5 rounded-xl border p-2">
                                    <ProductImage url={product.imageUrl} className="size-11 shrink-0 rounded-lg" iconClassName="size-[18px]" />
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-[13.5px] font-semibold">{product.name}</p>
                                        <p className="text-muted-foreground mt-0.5 text-xs">{money(product.priceCents)}</p>
                                    </div>
                                    {flag && (
                                        <span className={cn('rounded-full px-2 py-0.5 text-[10.5px] font-bold whitespace-nowrap', flag.className)}>
                                            {flag.label}
                                        </span>
                                    )}
                                </Link>
