...
export default function ProductForm({
    ...
    const keptImages = (product?.images ?? []).filter(
        (image) => !form.data.remove_image_ids.includes(image.id),
    );
    ...
    function submit(event: FormEvent) {
        ...
        if (product) {
            ...
            form.post(update.url({ product: product.id }), {
                forceFormData: true,
            });
        } else {
    ...
    return (
        <>
            ...
            <PageHeader
                title={title}
                back={product ? show.url({ product: product.id }) : index.url()}
            />

            <form
                onSubmit={submit}
                className="lg:bg-card lg:border-border flex flex-col gap-4 px-5 pt-2 lg:mx-5 lg:mt-2 lg:max-w-[600px] lg:rounded-2xl lg:border lg:p-7"
            >
                <div>
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        ...
                        {form.data.photos.map((_, position) => (
                            <div
                                key={previews[position]}
                                className="relative aspect-square overflow-hidden rounded-xl"
                            >
                                <img
                                    src={previews[position]}
                                    alt=""
                                    className="size-full object-cover"
                                />
                                <button
                ...
                <div className="grid grid-cols-2 gap-2.5">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <MoneyInput
                            id="cost"
                            cents={form.data.cost_cents}
                            onCentsChange={(cents) =>
                                form.setData('cost_cents', cents)
                            }
                            placeholder={t('products.form.costPlaceholder')}
                            className="h-11"
                        />
                    ...
                    {categories.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {categories.map((item) => (
                                <Chip
                                    key={item}
                                    active={form.data.category === item}
                                    onClick={() =>
                                        form.setData('category', item)
                                    }
                                >
                                    {item}
                                </Chip>
                ...
                <div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <InputError
                            message={
                                errors['variants.0.name'] ||
                                errors['variants.0.stock_qty']
                            }
                        />
                    </div>
                    <button
                ...
                <label className="flex items-center gap-2.5 text-sm">
                    <Checkbox
                        checked={form.data.catalog_visible}
                        onCheckedChange={(checked) =>
                            form.setData('catalog_visible', checked === true)
                        }
                    />
