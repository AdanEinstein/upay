...
export default function ReceiptViewer({
    ...
    function trackPointer(event: PointerEvent<HTMLDivElement>) {
        ...
        if (pinchDistance.current) {
            setZoom((current) =>
                clamp(current * (distance / pinchDistance.current!)),
            );
        }
    ...
    return (
        <Dialog open={open} onOpenChange={handleOpenChange}>
            <DialogContent
                className={cn(
                    brandScope,
                    'flex max-h-[92svh] flex-col gap-3 p-4 sm:max-w-3xl',
                )}
            >
                <DialogTitle className="pr-10 text-[15px] font-bold">
                    {t('sale.receipt.title')}
                </DialogTitle>
                <DialogDescription className="sr-only">
                    {t('sale.receipt.description')}
                </DialogDescription>
                ...
                {type === 'image' ? (
                    ...
                    <iframe
                        src={url}
                        title={t('sale.receipt.title')}
                        className="bg-muted h-[65svh] w-full rounded-xl"
                    />
                )}
                    ...
                    {type === 'image' ? (
                        <div className="flex items-center gap-1.5">
                            <Button
                                size="icon"
                                variant="outline"
                                aria-label={t('sale.receipt.zoomOut')}
                                disabled={zoom <= MIN_ZOOM}
                                onClick={() =>
                                    setZoom((current) => clamp(current - 0.5))
                                }
                            >
                                <MagnifyingGlassMinusIcon />
                            </Button>
                            <Button
                                ...
                                aria-label={t('sale.receipt.zoomReset')}
                                onClick={() => setZoom(1)}
                            >
                                {Math.round(zoom * 100)}%
                            </Button>
                            <Button
                                size="icon"
                                variant="outline"
                                aria-label={t('sale.receipt.zoomIn')}
